...
import com.yun.android.provider.RestClientBase;
import com.yun.android.provider.exceptions.NDULibraryAuthenticationException;
import com.yun.android.provider.exceptions.NDULibraryConnectionException;
import com.yun.android.provider.exceptions.NDULibraryParsingException;
...
public class LVoxRestClient extends RestClientBase {
	...
	public List<LVoxBook> getLatest(int pageNumber) throws NDULibraryConnectionException, NDULibraryParsingException {
		...
		String resp = connect(String.format(
				"http://librivox.org/api/feed/audiobooks?since=%d&limit=%d&offset=%d&extended=1", oneWeekAgo,
	...
	public List<LVoxBook> searchByTitle(String title) throws NDULibraryConnectionException, NDULibraryParsingException {
		try {
			String requestURL = String.format("http://librivox.org/api/feed/audiobooks/title/^%s?extended=1",
	...
	public List<LVoxBook> searchByAuthor(String author) throws NDULibraryConnectionException,
	...
	public List<LVoxBook> getBooksInCategory(String category) throws NDULibraryConnectionException,
			NDULibraryParsingException {
		// TODO: Currently the API doesn't work. Requesting clarification from
		// Librivox.
		try {
			String resp = connect(String.format("https://catalog.librivox.org/search_xml.php?extended=1&genre=%s",
	...
	// public void downloadBook(String zipFileURL, String saveLocation) throws
	// NDULibraryConnectionException, NDULibraryParsingException
	// {
	// downloadFile(zipFileURL, saveLocation);
	// }

	public URLConnection getDownloadChapterConnection(String url) throws NDULibraryConnectionException,
			NDULibraryParsingException {
			...
			while (eventType != XmlPullParser.END_DOCUMENT) {
				...
				if (eventType == XmlPullParser.START_TAG) {
					if (xpp.getName().equalsIgnoreCase("book")) {
						...
						// Don't add the book if the zip file URL isn't filled
						// out
